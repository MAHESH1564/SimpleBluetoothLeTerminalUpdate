...
object BluetoothUtil {
    ...
    private fun showSettingsDialog(fragment: Fragment) {
        val s = fragment.getString(R.string.permgrouplab_nearby_devices)
        val builder = AlertDialog.Builder(fragment.activity)
        builder.setTitle(fragment.getString(R.string.bluetooth_permission_title))
    ...
    fun hasPermissions(fragment: Fragment, requestPermissionLauncher: ActivityResultLauncher<Array<String>?>): Boolean {
        ...
        return if (missingPermissions) {
            if (showRationale) {
                showRationaleDialog(fragment) { _: DialogInterface?, _: Int -> requestPermissionLauncher.launch(permissions) }
            } else {
    ...
    fun onPermissionsResult(fragment: Fragment, grants: Map<String?, Boolean>, cb: PermissionGrantedCallback) {
        ...
        val showRationale = (fragment.shouldShowRequestPermissionRationale(Manifest.permission.BLUETOOTH_CONNECT)
                or fragment.shouldShowRequestPermissionRationale(Manifest.permission.BLUETOOTH_SCAN))
        val granted = grants.values.stream().reduce(true) { a: Boolean, b: Boolean -> a && b }
        if (granted) {
            cb.call()
