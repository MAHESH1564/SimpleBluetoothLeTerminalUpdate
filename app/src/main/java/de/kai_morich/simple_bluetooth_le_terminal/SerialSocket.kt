...
class SerialSocket(context: Context, device: BluetoothDevice?) : BluetoothGattCallback() {
    ...
    override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
        ...
        if (newState == BluetoothProfile.STATE_CONNECTED) {
            ...
            if (connected) onSerialIoError(IOException("gatt status $status")) else onSerialConnectError(IOException("gatt status $status"))
        } else {
    ...
    private fun connectCharacteristics1(gatt: BluetoothGatt) {
        ...
        if (delegate == null || readCharacteristic == null || writeCharacteristic == null) {
            for (gattService in gatt.services) {
                ...
                for (characteristic in gattService.characteristics) Log.d(TAG, "characteristic " + characteristic.uuid)
            }
        ...
        private fun grantReadCredits() {
            ...
            if (readCredits <= minReadCredits) {
                ...
                if (!gatt!!.writeCharacteristic(writeCreditsCharacteristic)) {
                    if (connected) onSerialIoError(IOException("write read credits failed")) else onSerialConnectError(IOException("write read credits failed"))
                }
            }
        }
    }
    ...
    companion object {
        ...
        private val BLUETOOTH_LE_TIO_CHAR_TX_CREDITS = UUID.fromString("00000003-0000-1000-8000-008025000000") // W
        private val BLUETOOTH_LE_TIO_CHAR_RX_CREDITS = UUID.fromString("00000004-0000-1000-8000-008025000000") // I
        private const val MAX_MTU = 512 // BLE standard does not limit, some BLE 4.2 devices support 251, various source say that Android has max 512
